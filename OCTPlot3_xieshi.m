%% load results;
load('E:\文章撰写\斜视猴子\OCT\OCTResults_Final.mat');
...
%% get x;
% Load OCT image;
[FileName, FilePath] = uigetfile({'*.tif'},'Select data informations','E:\文章撰写\斜视猴子\OCT\Left\');
RawData = imread(fullfile(FilePath,FileName));
OCTData = squeeze(RawData(:,:,1));
% plot piture;
figure;
imshow(adapthisteq(OCTData(:,504:1264), 'NumTiles',[20 20]))
